...
const DoctoralDashboard = () => {
  ...
  return (
    <MainLayout title="Spațiul Doctoral"><div className="mx-auto max-w-7xl px-4 py-6 sm:px-6">
      ...
      <Tabs defaultValue="journey"><TabsList className="mb-5 h-auto flex-wrap"><TabsTrigger value="journey">Parcurs și termene</TabsTrigger><TabsTrigger value="documents">{isManager ? "Documente" : "Documentele mele"}</TabsTrigger><TabsTrigger value="feedback">Feedback conducător</TabsTrigger></TabsList>
        <TabsContent value="feedback"><div className="space-y-3">{feedback.length === 0 ? <div className="border-y border-border py-12 text-center text-muted-foreground"><MessageCircle className="mx-auto mb-3 h-9 w-9" /><p>Încă nu ai primit observații de la conducător.</p></div> : feedback.map((note) => <div key={note.id} className="border-b border-border py-4"><p className="text-xs text-muted-foreground">{format(parseISO(note.created_at), 'd MMMM yyyy, HH:mm', { locale: ro })}</p><p className="mt-1 whitespace-pre-wrap text-sm">{note.body}</p></div>)}</div></TabsContent>
