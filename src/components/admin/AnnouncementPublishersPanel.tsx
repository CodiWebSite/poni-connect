...
import { Megaphone, Search, Loader2, Trash2, Plus, UserPlus } from 'lucide-react';
...
const AnnouncementPublishersPanel = () => {
  const { user } = useAuth();
  ...
  const fetchData = async () => {
    ...
    const [pubRes, profRes] = await Promise.all([
      supabase.from('announcement_publishers').select('id, user_id, created_at'),
    ...
    const enriched: Publisher[] = pubs.map(p => {
      ...
      return {
        ...p,
        full_name: prof?.full_name || 'Necunoscut',
  ...
  const addPublisher = async (userId: string) => {
    setAdding(true);
    const { error } = await supabase.from('announcement_publishers').insert({
      user_id: userId,
      added_by: user?.id,
    });
    if (error) {
      ...
      toast({ title: 'Adăugat', description: 'Utilizatorul poate acum posta anunțuri.' });
      await fetchData();
    }
    setAdding(false);
  };
  ...
  const removePublisher = async (id: string) => {
    const { error } = await supabase.from('announcement_publishers').delete().eq('id', id);
    if (error) {
      toast({ title: 'Eroare', description: 'Nu s-a putut elimina utilizatorul.', variant: 'destructive' });
    } else {
      toast({ title: 'Eliminat', description: 'Dreptul de a posta anunțuri a fost revocat.' });
      await fetchData();
    }
  };
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          Drepturi Publicare Anunțuri
        </CardTitle>
        <CardDescription>
          Utilizatorii cu roluri superioare (HR, Șefi, Admin, etc.) pot posta implicit. 
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-2">
          ...
          {loading ? (
            ...
            <p className="text-sm text-muted-foreground py-4 text-center">Nu există utilizatori adăugați manual. Doar cei cu roluri superioare pot posta.</p>
          ) : (
            <div className="space-y-2">
              {publishers.map(pub => (
...
export default AnnouncementPublishersPanel;
